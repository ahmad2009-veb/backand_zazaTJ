...
class RestaurantLogic
{
    ...
    public static function get_popular_restaurants($limit = 9, $offset = 0, $type = 'all')
    {
        $paginator = Restaurant::withOpen()
            ->with([
                'discount' => function ($q) {
            ...
            ->map(function (Restaurant $restaurant) {
                return Helpers::restaurant_data_formatting($restaurant);
            });
        // ->paginate($limit, ['*'], 'page', $offset);
        /*$paginator->count();*/
    ...
    public static function search_restaurants(
        $name,
        $zone_id,
        $category_id = null,
        $limit = 10,
    ...
    {
        $key = explode(' ', $name);
        $paginator = Restaurant::withOpen()->with([
            'discount' => function ($q) {
                return $q->validate();
            },
    ...
    {
        $monthly_earning = OrderTransaction::whereMonth('created_at', date('m'))->NotRefunded()->where('vendor_id',
            $vendor_id)->sum('restaurant_amount');
        $weekly_earning = OrderTransaction::whereBetween('created_at',
            [now()->startOfWeek(), now()->endOfWeek()])->NotRefunded()->where('vendor_id',
            $vendor_id)->sum('restaurant_amount');
        $daily_earning = OrderTransaction::whereDate('created_at', now())->NotRefunded()->where('vendor_id',
            $vendor_id)->sum('restaurant_amount');

        return [
            'monthely_earning' => (float)$monthly_earning,
            'weekly_earning' => (float)$weekly_earning,
            'daily_earning' => (float)$daily_earning,
        ];
    }
    ...
    public static function format_export_restaurants($restaurants)
    {
        ...
        foreach ($restaurants as $item) {
            ...
            $storage[] = [
                ...
                'restaurantName' => $item->stores[0]->name,
