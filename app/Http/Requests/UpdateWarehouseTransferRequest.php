...
class UpdateWarehouseTransferRequest extends FormRequest
{
    ...
    public function rules(): array
    {
        ...
        $currentTransfer = $this->route('transfer');
        $effectiveType = $transferType ?? $currentTransfer?->transfer_type?->value ?? 'internal';

        $rules = [
            'status' => 'nullable|in:pending,sent,received,completed,cancelled',
        ...
        // Conditional rules based on transfer type
        if ($effectiveType === 'internal') {
            $rules['to_warehouse_id'] = 'nullable|exists:warehouses,id';
        } else {
            $rules['to_vendor_id'] = 'nullable|exists:vendors,id';
            $rules['to_warehouse_id'] = 'nullable|exists:warehouses,id';
    ...
    {
        return [
            'status.in' => 'Статус должен быть: pending, sent, received, completed или cancelled',
            'transfer_type.in' => 'Тип перемещения должен быть: internal или external',
            ...
            'products.*.product_id.required_with' => 'ID товара обязателен',
            'products.*.product_id.exists' => 'Товар не найден',
            'products.*.quantity.required_with' => 'Количество обязательно',
    ...
    public function withValidator(Validator $validator): void
    {
        $validator->after(function ($validator) {
            ...
            // Only validate products if they are being updated
            if (!empty($products)) {
                $fromWarehouseId = $this->input('from_warehouse_id') ?? $currentTransfer?->from_warehouse_id;
            ...
            if ($effectiveType === 'external' && $isInstallment) {
