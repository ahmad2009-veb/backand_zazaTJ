...
class POSController extends Controller
{
    ...
    public function quick_view_card_item(Request $request)
    {
        ...
        return response()->json([
            'success' => 1,
            'view'    => view('vendor-views.pos._quick-view-cart-item',
                compact('product', 'cart_item', 'item_key'))->render(),
        ]);
    }
            ...
            for ($i = 0; $i < $count; $i++) {
                if (json_decode($product->variations)[$i]->type == $str) {
                    $price = json_decode($product->variations)[$i]->price - Helpers::product_discount_calculate($product,
                            json_decode($product->variations)[$i]->price, Helpers::get_restaurant_data());
                }
            }
        } else {
            ...
            if (is_array($c)) {
                ...
                if ($product) {
                    ...
                    $or_d                       = [
                        ...
                        'variation'          => json_encode(count($c['variations']) ? Helpers::get_varient($product->variations,
                            $c['variations']) : []),
