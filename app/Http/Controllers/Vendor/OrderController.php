...
class OrderController extends Controller
{
    ...
    {
        Order::where(['rest_checked' => 0])->where('restaurant_id', Helpers::get_restaurant_id())->update(['rest_checked' => 1]);
            ...
            ->when($status == 'all', function ($query) {
                return $query->where(function ($query) {
                    $query->whereNotIn('order_status',
                        (config('order_confirmation_model') == 'restaurant' || Helpers::get_restaurant_data()->self_delivery_system) ? [
                            'refund_requested',
                            'refunded',
    ...
    public function generate_invoice_print($id)
    {
        $order = Order::where(['id' => $id, 'restaurant_id' => Helpers::get_restaurant_id()])->first();
