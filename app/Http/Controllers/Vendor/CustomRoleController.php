...
class CustomRoleController extends Controller
{
    ...
    {
        $rl = EmployeeRole::where('restaurant_id',
            Helpers::get_restaurant_id())->orderBy('name')->paginate(config('default_pagination'));
    ...
    public function update(Request $request, $id)
    {
        ...
        DB::table('employee_roles')->where('restaurant_id',
            Helpers::get_restaurant_id())->where(['id' => $id])->update([
            'name'          => $request->name,
            'modules'       => json_encode($request['modules']),
            'status'        => 1,
            'restaurant_id' => Helpers::get_restaurant_id(),
