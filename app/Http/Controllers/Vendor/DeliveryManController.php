...
class DeliveryManController extends Controller
{
    ...
    {
        $delivery_men = DeliveryMan::where('restaurant_id',
            Helpers::get_restaurant_id())->latest()->paginate(config('default_pagination'));
    ...
    {
        $dm = DeliveryMan::with(['reviews'])->where('restaurant_id',
            Helpers::get_restaurant_id())->where(['id' => $id])->first();
        if ($tab == 'info') {
    ...
    public function get_deliverymen(Request $request)
    {
        ...
        $data     = DeliveryMan::when($zone_ids, function ($query) use ($zone_ids) {
            ...
            })->where('restaurant_id', Helpers::get_restaurant_id())->limit(8)->get([
                'id',
                DB::raw('CONCAT(f_name, " ", l_name) as text'),
            ]);
