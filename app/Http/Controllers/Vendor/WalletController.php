...
class WalletController extends Controller
{
    ...
    {
        $withdraw_req = WithdrawRequest::with(['vendor'])->where('vendor_id',
            Helpers::get_vendor_id())->latest()->paginate(config('default_pagination'));
    ...
    public function w_request(Request $request)
    {
        ...
        if ($w->balance >= $request['amount'] && $request['amount'] > .01) {
            ...
            RestaurantWallet::where('vendor_id', Helpers::get_vendor_id())->increment('pending_withdraw',
                $request['amount']);
    ...
    public function close_request($id)
    {
        ...
        if ($wr->approved == 0) {
            RestaurantWallet::where('vendor_id', Helpers::get_vendor_id())->decrement('pending_withdraw',
                $wr['amount']);
        }
        $wr->delete();
