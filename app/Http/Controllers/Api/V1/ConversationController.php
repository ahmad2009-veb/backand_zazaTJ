...
class ConversationController extends Controller
{
    public function messages_store(Request $request)
    {
        ...
        if ($request->conversation_id) {
            ...
            if ($request->receiver_type == 'admin') {
                ...
                if ($request->receiver_type == 'vendor') {
                    ...
                    if ($request->receiver_type == 'delivery_man') {
        ...
        try {
            ...
            $conversation->save();
            {
                if ($request->receiver_type == 'admin' || $receiver_id == 0) {
                    ...
                    if ($request->receiver_type == 'vendor' || $request->receiver_type == 'delivery_man') {
                        $data = [
                            'title'           => trans('messages.message'),
                            'description'     => trans('messages.message_description'),
                            'order_id'        => '',
        ...
        $messages = Message::where(['conversation_id' => $conversation->id])->latest()->paginate($limit, ['*'], 'page',
            $offset);
        ...
        if ($conv->sender_type == 'vendor' && $conversation->sender) {
            ...
            $order = Order::where('user_id', $request->user()->id)->where('restaurant_id',
                $vd->restaurants[0]->id)->whereIn('order_status',
                ['pending', 'accepted', 'confirmed', 'processing', 'handover', 'picked_up'])->count();
        } else {
            if ($conv->receiver_type == 'vendor' && $conversation->receiver) {
                $vd    = Vendor::find($conv->receiver->vendor_id);
                $order = Order::where('user_id', $request->user()->id)->where('restaurant_id',
                    $vd->restaurants[0]->id)->whereIn('order_status',
                    ['pending', 'accepted', 'confirmed', 'processing', 'handover', 'picked_up'])->count();
            } else {
                if ($conv->sender_type == 'delivery_man' && $conversation->sender) {
                    $user2 = DeliveryMan::find($conv->sender->deliveryman_id);
                    $order = Order::where('user_id', $request->user()->id)->where('delivery_man_id',
                        $user2->id)->whereIn('order_status',
                        ['pending', 'accepted', 'confirmed', 'processing', 'handover', 'picked_up'])->count();
                } else {
                    if ($conv->receiver_type == 'delivery_man' && $conversation->receiver) {
    ...
    public function conversations(Request $request)
    {
        ...
        $conversations = Conversation::with('sender', 'receiver',
            'last_message')->where(['sender_id' => $sender->id])->orWhere(['receiver_id' => $sender->id])->orderBy('last_message_time',
            'DESC')->paginate($limit, ['*'], 'page', $offset);
    ...
    public function get_searched_conversations(Request $request)
    {
        ...
        $conversations = Conversation::with('sender', 'receiver',
            'last_message')->WhereUser($sender->id)->where(function ($qu) use ($key) {
    ...
    public function messages(Request $request)
    {
        ...
        if ($request->conversation_id) {
            ...
            if ($request->has('admin_id')) {
                $conversation = Conversation::with(['sender', 'receiver', 'last_message'])->WhereConversation($user->id,
                    0)->first();
                $order        = 0;
            } else {
                if ($request->vendor_id) {
                    ...
                    $conversation = Conversation::with([
                        ...
                        'receiver',
                        'last_message',
                    ])->WhereConversation($user->id, $vendor->id)->first();
                } else {
                    if ($request->delivery_man_id) {
                        ...
                        $conversation = Conversation::with([
                            ...
                            'receiver',
                            'last_message',
                        ])->WhereConversation($user->id, $dm->id)->first();
                    }
                }
            }
        }
        ...
        if (isset($conversation)) {
            if ($conversation->sender_type == 'vendor' && $conversation->sender) {
                ...
                $order = Order::where('user_id', $user->user_id)->where('restaurant_id',
                    $vd->restaurants[0]->id)->whereIn('order_status',
                    ['pending', 'accepted', 'confirmed', 'processing', 'handover', 'picked_up'])->count();
            } else {
                if ($conversation->receiver_type == 'vendor' && $conversation->receiver) {
                    $vd    = Vendor::find($conversation->receiver->vendor_id);
                    $order = Order::where('user_id', $user->user_id)->where('restaurant_id',
                        $vd->restaurants[0]->id)->whereIn('order_status',
                        ['pending', 'accepted', 'confirmed', 'processing', 'handover', 'picked_up'])->count();
                } else {
                    if ($conversation->sender_type == 'delivery_man' && $conversation->sender) {
                        $user2 = DeliveryMan::find($conversation->sender->deliveryman_id);
                        $order = Order::where('user_id', $user->user_id)->where('delivery_man_id',
                            $user2->id)->whereIn('order_status',
                            ['pending', 'accepted', 'confirmed', 'processing', 'handover', 'picked_up'])->count();
                    } else {
                        if ($conversation->receiver_type == 'delivery_man' && $conversation->receiver) {
            ...
            Message::where(['conversation_id' => $conversation->id])->where('sender_id', '!=',
                $user->id)->update(['is_seen' => 1]);
            $messages = Message::where(['conversation_id' => $conversation->id])->latest()->paginate($limit, ['*'],
                'page', $offset);
        } else {
            $messages = [];
            $order    = 0;
    ...
    public function dm_messages_store(Request $request)
    {
        ...
        if ($conv->sender_type == 'vendor' && $conversation->sender) {
            $vd    = Vendor::find($conv->sender->vendor_id);
            $order = Order::where('delivery_man_id', $dm->id)->where('restaurant_id',
                $vd->restaurants[0]->id)->whereIn('order_status',
                ['pending', 'accepted', 'confirmed', 'processing', 'handover', 'picked_up'])->count();
        } else {
            if ($conv->receiver_type == 'vendor' && $conversation->receiver) {
                ...
                if ($conv->sender_type == 'customer' && $conversation->sender) {
                    ...
                    $order = Order::where('delivery_man_id', $dm->id)->where('user_id',
                        $user->id)->whereIn('order_status',
                        ['pending', 'accepted', 'confirmed', 'processing', 'handover', 'picked_up'])->count();
                } else {
                    if ($conv->receiver_type == 'customer' && $conversation->receiver) {
    ...
    public function dm_messages(Request $request)
    {
        ...
        if ($request->conversation_id) {
            ...
            if ($request->vendor_id) {
                ...
                $conversation = Conversation::with([
                    ...
                    'receiver',
                    'last_message',
                ])->WhereConversation($delivery_man->id, $vendor->id)->first();
                ...
                if ($request->user_id) {
                    ...
                    $conversation = Conversation::with([
                        ...
                        'receiver',
                        'last_message',
                    ])->WhereConversation($delivery_man->id, $user->id)->first();
                }
            }
        }
        ...
        if ($conversation) {
            ...
            if ($conversation->sender_type == 'vendor' && $conversation->sender) {
                ...
                if ($conversation->receiver_type == 'vendor' && $conversation->receiver) {
                    ...
                    if ($conversation->sender_type == 'customer' && $conversation->sender) {
                        ...
                        if ($conversation->receiver_type == 'customer' && $conversation->receiver) {
            ...
            Message::where(['conversation_id' => $conversation->id])->where('sender_id', '!=',
                $delivery_man->id)->update(['is_seen' => 1]);
            $messages = Message::where(['conversation_id' => $conversation->id])->latest()->paginate($limit, ['*'],
                'page', $offset);
        } else {
            $messages = [];
