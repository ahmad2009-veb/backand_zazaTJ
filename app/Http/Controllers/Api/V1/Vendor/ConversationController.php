...
class ConversationController extends Controller
{
    public function messages_store(Request $request)
    {
        ...
        $conversation = Conversation::with('sender', 'receiver', 'last_message')->WhereConversation($sender->id,
            $receiver_id)->first();
        ...
        if ($conv->sender_type == 'customer' && $conversation->sender) {
            ...
            $order = Order::where('restaurant_id', $vendor->restaurants[0]->id)->where('user_id',
            ...
            if ($conv->receiver_type == 'customer' && $conversation->receiver) {
                ...
                if ($conv->sender_type == 'delivery_man' && $conversation->sender) {
                    ...
                    $order = Order::where('restaurant_id', $vendor->restaurants[0]->id)->where('delivery_man_id',
    ...
    public function conversations(Request $request)
    {
        ...
        $conversations = Conversation::with([
            ...
            'receiver',
            'last_message',
        ])->where(['sender_id' => $sender->id])->orWhere(['receiver_id' => $sender->id])->orderBy('last_message_time',
    ...
    public function messages(Request $request)
    {
        ...
        if ($conversation) {
            ...
            if ($conversation->sender_type == 'customer' && $conversation->sender) {
                ...
                $order = Order::where('restaurant_id', $vnd->restaurants[0]->id)->where('user_id',
                    $user->id)->whereIn('order_status',
                    ['pending', 'accepted', 'confirmed', 'processing', 'handover', 'picked_up'])->count();
            } else {
                if ($conversation->receiver_type == 'customer' && $conversation->receiver) {
                    ...
                    if ($conversation->sender_type == 'delivery_man' && $conversation->sender) {
                        ...
                        $order = Order::where('restaurant_id', $vnd->restaurants[0]->id)->where('delivery_man_id',
                            $user2->id)->whereIn('order_status',
                            ['pending', 'accepted', 'confirmed', 'processing', 'handover', 'picked_up'])->count();
                    } else {
