...
class BusinessSettingsController extends Controller
{
    ...
    public function update_restaurant_setup(Request $request)
    {
        ...
        $validator->sometimes('per_km_delivery_charge', 'required_with:minimum_delivery_charge',
            function ($request) use ($restaurant) {
                return ($restaurant->self_delivery_system);
            });
        $validator->sometimes('minimum_delivery_charge', 'required_with:per_km_delivery_charge',
            function ($request) use ($restaurant) {
                return ($restaurant->self_delivery_system);
            });
        ...
        if (!$request->take_away && !$request->delivery) {
            return response()->json([
                'error' => [
                    [
                        'code'    => 'delivery_or_take_way',
                        'message' => trans('messages.can_not_disable_both_take_away_and_delivery'),
                    ],
                ],
            ], 403);
        }
    ...
    public function add_schedule(Request $request)
    {
        ...
        $temp       = RestaurantSchedule::where('day', $request->day)->where('restaurant_id', $restaurant->id)
            ->where(function ($q) use ($request) {
                return $q->where(function ($query) use ($request) {
                    return $query->where('opening_time', '<=', $request->opening_time)->where('closing_time', '>=',
                        $request->opening_time);
                })->orWhere(function ($query) use ($request) {
                    return $query->where('opening_time', '<=', $request->closing_time)->where('closing_time', '>=',
        ...
        $restaurant_schedule = RestaurantSchedule::insertGetId([
        ...
        return response()->json([
            'message' => trans('messages.Schedule added successfully'),
            'id'      => $restaurant_schedule,
        ], 200);
    }
