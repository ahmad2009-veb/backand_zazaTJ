...
class FoodController extends Controller
{
    ...
    public function store(Request $request)
    {
        ...
        if (count($data) < 1) {
            $validator->getMessageBag()->add('translations',
                trans('messages.Name and description in english is required'));
        }
        ...
        $food->add_ons               = $request->has('addon_ids') ? json_encode(explode(',',
            $request->addon_ids)) : json_encode([]);
    ...
    public function update(Request $request)
    {
        ...
        $p->add_ons               = $request->has('addon_ids') ? json_encode(explode(',',
            $request->addon_ids)) : json_encode([]);
        $p->veg                   = $request->veg;
        $p->save();
