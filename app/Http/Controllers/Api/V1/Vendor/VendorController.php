...
class VendorController extends Controller
{
    ...
    public function active_status(Request $request)
    {
        ...
        return response()->json(['message' => $restaurant->active ? trans('messages.restaurant_opened') : trans('messages.restaurant_temporarily_closed')],
            200);
    }
    ...
    public function update_order_status(Request $request)
    {
        ...
        if ($order->picked_up != null) {
            return response()->json([
                'errors' => [
                    [
                        'code'    => 'status',
                        'message' => trans('messages.You_can_not_change_status_after_picked_up_by_delivery_man'),
                    ],
                ],
            ], 403);
        }
        ...
        if ($request->status == 'processing') {
            $order->processing_time = isset($request->processing_time) ? $request->processing_time : explode('-',
                $order['restaurant']['delivery_time'])[0];
        }
    ...
    public function get_notifications(Request $request)
    {
        ...
        $user_notifications = UserNotification::where('vendor_id', $vendor->id)->where('created_at', '>=',
    ...
    public function get_products(Request $request)
    {
        ...
        $paginator = Food::withoutGlobalScope('translate')->type($type)->where('restaurant_id',
            $request['vendor']->restaurants[0]->id)->latest()->paginate($limit, ['*'], 'page', $offset);
        $data      = [
            'total_size' => $paginator->total(),
            'limit'      => $limit,
            'offset'     => $offset,
    ...
    public function remove_account(Request $request)
    {
        ...
        if (Order::where('restaurant_id', $vendor->restaurants[0]->id)->whereIn('order_status',
