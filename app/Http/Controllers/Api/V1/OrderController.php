...
class OrderController extends Controller
{
    ...
    public function place_order(Request $request)
    {
        ...
        foreach ($request['cart'] as $c) {
            if ($c['item_campaign_id'] != null) {
                ...
                if ($product) {
                    ...
                    $product                    = Helpers::product_data_formatting($product, false, false,
                        app()->getLocale());
                    $addon_data                 = Helpers::calculate_addon_price(\App\Models\AddOn::whereIn('id',
                        $c['add_on_ids'])->get(), $c['add_on_qtys']);
                    $or_d                       = [
                        'food_id'            => null,
                ...
                if ($product) {
                    ...
                    $or_d                       = [
                        ...
                        'tax_amount'         => round(Helpers::tax_calculate($product, $price),
        ...
        $coupon_discount_amount = $coupon ? CouponLogic::get_discount($coupon,
            $product_price + $total_addon_price - $restaurant_discount_amount) : 0;
        ...
        if ($restaurant->minimum_order > $product_price + $total_addon_price) {
            return response()->json([
                'errors' => [
                    [
                        'code'    => 'order_time',
                        'message' => trans('messages.you_need_to_order_at_least',
        ...
        try {
            ...
            if ($request->payment_method == 'wallet') {
                CustomerLogic::create_wallet_transaction($order->user_id, $order->order_amount, 'order_place',
                    $order->id);
            }
    ...
    public function get_order_list(Request $request)
    {
        ...
        $paginator = Order::with([
            'restaurant',
            'delivery_man.rating',
        ])->withCount('details')->where(['user_id' => $request->user()->id])->whereIn('order_status', [
            'delivered',
            'canceled',
            'refund_requested',
            'refunded',
    ...
    public function get_running_orders(Request $request)
    {
        ...
        $paginator = Order::with([
            'restaurant',
            'delivery_man.rating',
        ])->withCount('details')->where(['user_id' => $request->user()->id])->whereNotIn('order_status', [
            'delivered',
            'canceled',
            'refund_requested',
            'refunded',
        ...
        if ($order) {
            ...
            try {
                ...
                if ($order->order_type == 'delivery' && !$order->scheduled) {
                    ...
                    Helpers::send_push_notif_to_topic($data, $order->restaurant->zone->deliveryman_wise_topic,
                        'order_request');
                }
            ...
            return response()->json(['message' => trans('messages.payment') . ' ' . trans('messages.method') . ' ' . trans('messages.updated_successfully')],
                200);
        }
