...
class DeliverymanController extends Controller
{
    ...
    public function get_current_orders(Request $request)
    {
        ...
        $orders = Order::with(['customer', 'store'])
            ->select('*', DB::raw('CAST(order_amount AS UNSIGNED) as total, order_status AS status'))
        ...
        $orders = $orders->map(function ($order) {
            // You must have $order->details or $order->products
        ...
        // return \App\Http\Resources\Vendor\OrderResource::collection($orders)->resolve();
    ...
    public function get_latest_orders(Request $request)
    {
        ...
        $orders = Order::with(['customer', 'stores']);

        // if ($dm->type == 'zone_wise') {
        //     $orders = $orders->whereHas('stores', function ($q) use ($dm) {
    ...
    public function accept_order(Request $request)
    {
            ...
            return response()->json([
                'errors' => [
                    [
                        ...
                        'message' => trans('messages.dm_maximum_order_exceed_warning'),
                    ],
                ],
            ], 405);
        }
        ...
        if ($order->payment_method == "cash_on_delivery" && (($cash_in_hand + $order->order_amount) >= $value)) {
            return response()->json([
                'errors' => Helpers::error_formater('dm_max_cash_in_hand',
                    translate('delivery man max cash in hand exceeds')),
            ], 203);
        }

        $order->order_status    = in_array($order->order_status,
            ['pending', 'confirmed']) ? 'accepted' : $order->order_status;
    ...
    public function get_order(Request $request)
    {
        ...
        $order = Order::with(['customer', 'restaurant', 'details'])->where([
            'delivery_man_id' => $dm['id'],
            'id'              => $request['order_id'],
        ])->Notpos()->first();
        if (!$order) {
    ...
    public function get_notifications(Request $request)
    {
        ...
        $user_notifications = UserNotification::where('delivery_man_id', $dm->id)->where('created_at', '>=',
            \Carbon\Carbon::today()->subDays(7))->get();
    ...
    public function remove_account(Request $request)
    {
        ...
        if (Order::where('delivery_man_id', $dm->id)->whereIn('order_status',
            ['pending', 'accepted', 'confirmed', 'processing', 'handover', 'picked_up'])->count()) {
            return response()->json([
                'errors' => [
                    [
                        'code'    => 'on-going',
                        'message' => translate('messages.user_account_delete_warning'),
                    ],
                ],
            ], 203);
        }
        ...
        if ($dm->wallet && $dm->wallet->collected_cash > 0) {
            return response()->json([
                'errors' => [
                    [
                        'code'    => 'on-going',
                        'message' => translate('messages.user_account_wallet_delete_warning'),
                    ],
                ],
            ], 203);
        }
    ...
    public function orderStatus(Request $request){
        $dm = DeliveryMan::with('store')->where(['auth_token' => $request['token']])->first();
...
{
    // Validate the incoming data (optional - currently commented)
    // $request->validate([
    //     'id' => 'required|exists:orders,id',
    //     'products' => 'required|array|min:1',
    //     'products.*.product_id' => 'required|integer',
    //     'products.*.quantity' => 'required|integer|min:1',
    //     'products.*.status' => 'required|string|in:delivered,canceled',
    //     'individual' => 'required|boolean',
    // ]);

    $order = Order::with(['details.product'])->where('id', $request->id)->firstOrFail();
    $details = $order->details;

    foreach ($request->products as $item) {
        $detail = $details->firstWhere('id', $item['product_id']);

        if (!$detail) continue;

        $originalQty = $detail->quantity;
        $reportedQty = $item['quantity'];
        $status = $item['status'];
       
        $detail->quantity = $reportedQty;
        $detail->save();
       ...
       $guard = request()->bearerToken() && auth('vendor_api')->check() ? 'vendor' :
        ...
        if ($originalQty !== $reportedQty) {
            OrderDeliveryAuditLog::create([
                'order_id' => $order->id,
                'product_id' => $detail->product_id,
                'original_quantity' => $originalQty,
                'new_quantity' => $reportedQty,
                'action' => 'updated',
                'actor_id' => $actorId,
                'actor_role' => $guard,
            ]);
        ...
        $unsoldQty = max(0, $originalQty - $reportedQty);

        
        if ($status === 'canceled') {
            $unsoldQty = $originalQty;
        }

        if ($unsoldQty > 0 && $detail->product && $detail->product->quantity !== null) {
            $detail->product->quantity += $unsoldQty;
            $detail->product->save();
    ...
    $order->order_amount = $order->details->sum(function ($d) {
        return $d->quantity * $d->price;
    });

    
    $productStatuses = collect($request->products)->pluck('status');
    
    if ($productStatuses->every(fn($s) => $s === 'canceled')) {
        $order->order_status = 'canceled';
        $order->canceled = now();
    }
    
    elseif ($productStatuses->contains('delivered')) {
    ...
    OrderStatusHistory::create([
        ...
        'vendor_id' => $order->store->vendor_id ?? null,
        'comment' => 'Courier updated quantities and statuses',
    ]);

    return response()->json(['message' => 'Статус заказа обновлен и остаток возвращён на склад.'], 200);
}
