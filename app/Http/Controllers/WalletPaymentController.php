...
class WalletPaymentController extends Controller
{
    ...
    public function make_payment(Request $request)
    {
        if (BusinessSetting::where('key', 'wallet_status')->first()->value != 1) {
            return Toastr::error(trans('messages.customer_wallet_disable_warning'));
        }
        ...
        $transaction = CustomerLogic::create_wallet_transaction($order->user_id, $order->order_amount, 'order_place',
            $order->id);
