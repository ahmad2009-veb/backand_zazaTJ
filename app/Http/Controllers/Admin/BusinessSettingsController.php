...
class BusinessSettingsController extends Controller
{
    ...
    public function business_index()
    {
        ...
        return view('admin-views.business-settings.business-index', compact('zones'));
    }
    ...
    public function business_setup(Request $request)
    {
        ...
        DB::table('business_settings')->updateOrInsert(['key' => 'default_zone'], [
            'value' => $request['default_zone'],
        ]);
    ...
    public function payment_update(Request $request, $name)
    {
        ...
        } elseif ($name == 'alif_payment') {
            $payment = BusinessSetting::where('key', 'alif_payment')->first();
            if (isset($payment) == false) {
                DB::table('business_settings')->insert([
                    'key'        => 'alif_payment',
                    'value'      => json_encode([
                ...
                DB::table('business_settings')->where(['key' => 'alif_payment'])->update([
                    'key'        => 'alif_payment',
                    'value'      => json_encode([
                        'status' => $request['status'],
                        'key'    => $request['key'],
                        ...
                        if ($tab == 'testimonial') {
                            ...
                            if ($tab == 'image') {
                                ...
                                if ($request->has('feature_section_image')) {
                                    $imageName = \Carbon\Carbon::now()->toDateString() . "-" . uniqid() . ".png";
                                    $request->feature_section_image->move(public_path('assets/landing/image'),
                                        $imageName);
                                    $data['feature_section_image'] = $imageName;
                                }
                                if ($request->has('mobile_app_section_image')) {
                                    $imageName = \Carbon\Carbon::now()->toDateString() . "-" . uniqid() . ".png";
                                    $request->mobile_app_section_image->move(public_path('assets/landing/image'),
                                        $imageName);
    ...
    public function config_update(Request $request, $service)
    {
        if ($service == 'google') {
        ...
        if ($service == 'yandex') {
            DB::table('business_settings')->updateOrInsert(['key' => 'yandex_map_api_key'], [
                'value' => $request['yandex_map_api_key'],
            ]);
        }
