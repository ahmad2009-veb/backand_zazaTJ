...
class DeliveryManController extends Controller
{
            ...
            if ($tab == 'transaction') {
                ...
                if ($tab == 'timelog') {
                    ...
                    if ($tab == 'conversation') {
                                ...
                                $conversations = $conversations->whereHas('sender', function ($query) use ($key) {
                                    foreach ($key as $value) {
                                        $query->where('f_name', 'like', "%{$value}%")->orWhere('l_name', 'like',
                                            "%{$value}%")->orWhere('phone', 'like', "%{$value}%")->orWhere('email',
                                            'like', "%{$value}%");
                                    }
                        ...
                        return view('admin-views.delivery-man.view.conversations',
                            compact('conversations', 'delivery_man', 'dm'));
                    }
                }
            }
        }
    ...
    public function update_application(Request $request)
    {
        ...
        try {
            if (config('mail.status')) {
                Mail::to($request['email'])->send(new \App\Mail\SelfRegistration($request->status,
    ...
    public function get_account_data(DeliveryMan $deliveryman)
    {
        ...
        if ($wallet) {
            ...
            $balance      = round($wallet->total_earning - $wallet->total_withdrawn - $wallet->pending_withdraw,
        ...
        if ($user) {
            ...
            if ($request->query('key')) {
                ...
                $conversations = $conversations->whereHas('sender', function ($query) use ($key) {
                    foreach ($key as $value) {
                        $query->where('f_name', 'like', "%{$value}%")->orWhere('l_name', 'like',
                            "%{$value}%")->orWhere('phone', 'like', "%{$value}%")->orWhere('email', 'like',
                            "%{$value}%");
                    }
                })->orWhereHas('receiver', function ($query1) use ($key) {
                    foreach ($key as $value) {
    ...
    public function get_conversation_list(Request $request)
    {
        ...
        $view = view('admin-views.delivery-man.partials._conversation_list',
            compact('conversations', 'delivery_man'))->render();
    ...
    public function conversation_view($conversation_id, $user_id)
    {
        ...
        return response()->json([
            'view' => view('admin-views.delivery-man.partials._conversations',
