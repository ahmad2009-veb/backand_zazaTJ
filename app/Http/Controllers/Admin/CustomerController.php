...
class CustomerController extends Controller
{
    public function customer_list(Request $request)
    {
        ...
        $allCustomers = User::when(count($key) > 0, function ($query) use ($key) {
            foreach ($key as $value) {
                $query->orWhere('f_name', 'like', "%{$value}%")
                    ->orWhere('l_name', 'like', "%{$value}%")
                    ->orWhere('email', 'like', "%{$value}%")
        ...
        return view('admin-views.customer.list', compact('customers', 'allCustomers'));
    }
    ...
    public function edit($id)
    {
        ...
        return view('admin-views.customer.customer-edit', compact('user', 'lastAddress'));
    }
    ...
    public function get_customers(Request $request)
    {
        ...
        if ($request->all) {
            $data[] = (object)['id' => false, 'text' => trans('messages.all')];
        }
