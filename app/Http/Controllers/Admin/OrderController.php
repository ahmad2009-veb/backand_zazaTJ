...
class OrderController extends Controller
{
    ...
    public function list($status, Request $request)
    {
        ...
        $orders = Order::with(['customer', 'restaurant'])
            ...
            ->when(
                ($status != 'all' && $status != 'scheduled' && $status != 'canceled' && $status != 'refund_requested' && $status != 'refunded' && $status != 'delivered' && $status != 'failed'),
                function ($query) {
    ...
    public function update(Request $request, Order $order)
    {
        ...
        if (session()->has('selected_restaurant')) {
            $copy = session()->get('selected_restaurant');
            if (array_key_exists("$order->id", $copy)) {
                $restaurant = $copy["$order->id"];
                if ($restaurant->id != $order->restaurant_id) {
                    $order->details()->delete();
                }
                unset($copy["$order->id"]);
            }
            session()->put('selected_restaurant', $copy);
//            $restaurantOrder = $restaurant->session()->get('selected_restaurant');
//            if ($restaurantOrder['order_id'] == $order->id) {
...
//        $restaurant = $request->session()->get('selected_restaurant', $order->restaurant);
        $coupon = null;
        $total_addon_price = 0;
        $product_price = 0;
...
//        if ($request->session()->has('selected_restaurant')) {
//            if ($request->session()->get('selected_restaurant')->id != $order->restaurant_id) {
//                $order->details()->delete();
//            }
//        }
        foreach ($cart as $c) {
        ...
        $coupon_discount_amount = $coupon ? CouponLogic::get_discount(
            $coupon,
            $product_price + $total_addon_price - $restaurant_discount_amount
        ) : 0;
        ...
        if ($restaurant->minimum_order > $product_price + $total_addon_price) {
            Toastr::error(
                trans(
                    'messages.you_need_to_order_at_least',
                    ['amount' => $restaurant->minimum_order . ' ' . Helpers::currency_code()]
                )
            );
    ...
    public function dispatch_list($status, Request $request)
    {
        ...
        return view(
            'admin-views.order.distaptch_list',
            compact(
                'orders',
                'status',
                'orderstatus',
    ...
    public function details(Request $request, $id)
    {
        ...
        $orderStatusHistories = auth('admin')->user()->role_id === 1 ? $order->statusHistories()->with('admin')->get() : null;
        if (isset($order)) {
            if (isset($order->restaurant) && $order->restaurant->self_delivery_system) {
                ...
                if ($order->restaurant !== null) {
                    $deliveryMen = DeliveryMan::where(
                        'zone_id',
                        $order->restaurant->zone_id
            ...
            $key = explode(' ', $keyword);
//            $selectedRestaurant = $request->session()->has('selected_restaurant') ? $request->session()->get('selected_restaurant')['restaurant'] : $order->restaurant;
            $selectedRestaurant = $order->restaurant;
            if ($request->session()->has('selected_restaurant')) {
                $restaurantOrder = $request->session()->get('selected_restaurant');
                if (array_key_exists("$order->id", $restaurantOrder)) {
                    if ($restaurantOrder["$order->id"]) {
                        $selectedRestaurant = $restaurantOrder["$order->id"];
                    }
                }
//                if ($restaurantOrder['order_id'] == $order->id) {
//                    $selectedRestaurant = $restaurantOrder['restaurant'];
//                }
            }
            $products = Food::withoutGlobalScope(RestaurantScope::class)->where(
                'restaurant_id',
...
//                $editing = true;
                ...
                } elseif ($cart[0]->order_id == $order->id) {
                    $editing = true;
                }
...
//                }
            ...
            $anotherRestaurants = Restaurant::query()->where('zone_id', $order->restaurant->zone_id)->get();

            return view(
                'admin-views.order.order-view',
                compact('order', 'deliveryMen', 'categories', 'products', 'category', 'keyword', 'editing', 'orderStatusHistories', 'anotherRestaurants', 'selectedRestaurant')
            );
        } else {
    ...
    public function changeRestaurant(Request $request)
    {
        ...
        $restaurant = Restaurant::query()->find((int) $request->restaurant_id);
        if ($order->restaurant_id != $request->restaurant_id) {
            $request->session()->put('order_cart', collect([]));
        }
        $copy = session()->get('selected_restaurant', []);
        $copy["$order->id"] = $restaurant;
        session()->put('selected_restaurant', $copy);
//        $request->session()->put('selected_restaurant', [
//            "$order->id" => $restaurant,
//        ]);
        return back();
    }
        ...
        if ($request->order_status == 'delivered') {
            ...
            if ($request->order_status == 'refunded') {
                ...
                if ($request->order_status == 'canceled') {
                    ...
                    $this->notificationService->sendFirebaseOrderStatusNotification('order_cancled_message', $order);
                }
            }
        }
    ...
    public function add_delivery_man($order_id, $delivery_man_id)
    {
        ...
        if ($deliveryman) {
            ...
            $order->order_status = in_array(
                $order->order_status,
                ['pending', 'confirmed']
            ) ? 'accepted' : $order->order_status;
            $order->accepted = now();
            $order->save();
            ...
            // Send Firebase notification for dm accepted status
            $this->notificationService->sendFirebaseOrderStatusNotification('out_for_delivery_message', $order);
    ...
    public function add_to_cart(Request $request)
    {
        ...
        if ($str != null) {
            ...
            $data['variation'] = json_encode([$str], JSON_UNESCAPED_UNICODE);
        } else {
            $price = $product->price;
        }
        ...
        $addon_data = Helpers::calculate_addon_price(
            \App\Models\AddOn::whereIn('id', $add_ons)->get(),
            $add_on_qtys
        );
    ...
    public function edit(Request $request, Order $order)
    {
        ...
        if ($request->session()->has('order_cart')) {
            ...
            $request->session()->put('order_cart', $cart);
//            $request->session()->put('selected_restaurant', $order->restaurant);
            if ($request->session()->has('selected_restaurant')) {
                $copy = session()->get('selected_restaurant');
                if (array_key_exists("$order->id", $copy)) {
                    unset($copy[$order->id]);
    ...
    public function quick_view(Request $request)
    {
        ...
        return response()->json([
            ...
            'view' => view(
                ...
                compact('product', 'order_id', 'item_type')
            )->render(),
        ]);
    }
    ...
    public function quick_view_cart_item(Request $request)
    {
        ...
        return response()->json([
            ...
            'view' => view(
                ...
                compact('order_id', 'product', 'cart_item', 'item_key', 'item_type')
            )->render(),
        ]);
    }
