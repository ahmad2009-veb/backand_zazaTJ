...
class CategoryController extends Controller
{
    ...
    function store(Request $request)
    {
        ...
        $category->image     = $request->has('image') ? Helpers::upload('category/', 'png',
            $request->file('image')) : 'def.png';
    ...
    public function popular(Request $request)
    {
        ...
        Toastr::success(trans('messages.category_popular_flag_updated'));
    ...
    public function update(Request $request, $id)
    {
        ...
        $category->name  = $request->name[array_search('en', $request->lang)];
        $category->image = $request->has('image') ? Helpers::update('category/', $category->image, 'png',
            $request->file('image')) : $category->image;
        $category->save();
        foreach ($request->lang as $index => $key) {
            if ($request->name[$index] && $key != 'en') {
    ...
    public function get_all(Request $request)
    {
        $data = Category::where('name', 'like',
            '%' . $request->q . '%')->limit(8)->get([DB::raw('id, CONCAT(name, " (", if(position = 0, "' . trans('messages.main') . '", "' . trans('messages.sub') . '"),")") as text')]);
