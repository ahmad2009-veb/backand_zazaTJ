...
class VendorController extends Controller
{
    ...
    public function update(Request $request, Restaurant $restaurant)
    {
        ...
        $restaurant->email         = $request->email;
        $restaurant->phone         = $request->phone;
        $restaurant->logo          = $request->has('logo') ? Helpers::update('restaurant/', $restaurant->logo, 'png',
            $request->file('logo')) : $restaurant->logo;
        $restaurant->cover_photo   = $request->has('cover_photo') ? Helpers::update('restaurant/cover/',
            $restaurant->cover_photo, 'png', $request->file('cover_photo')) : $restaurant->cover_photo;
        $restaurant->name          = $request->name;
        $restaurant->address       = $request->address;
        $restaurant->map_link      = $request->map_link;
        $restaurant->latitude      = $request->latitude;
                        ...
                        if ($tab == 'transaction') {
                            ...
                            if ($tab == 'reviews') {
                                ...
                                if ($tab == 'conversations') {
                                    ...
                                    return view('admin-views.vendor.view.conversations',
                                        compact('restaurant', 'sub_tab', 'conversations'));
                                }
                            }
                        }
                    }
    ...
    public function get_restaurants(Request $request)
    {
        ...
        $data     = Restaurant::join('zones', 'zones.id', '=', 'restaurants.zone_id')
            ->when($zone_ids, function ($query) use ($zone_ids) {
                ...
                '%' . $request->q . '%')->limit(8)->get([DB::raw('restaurants.id as id, CONCAT(restaurants.name, " (", zones.name,")") as text')]);
    ...
    public function update_application(Request $request)
    {
        ...
        try {
            if (config('mail.status')) {
                Mail::to($request['email'])->send(new \App\Mail\SelfRegistration($request->status == 1 ? 'approved' : 'denied',
    ...
    public function withdrawStatus(Request $request, $id)
    {
        ...
        if ($request->approved == 1) {
            ...
            RestaurantWallet::where('vendor_id', $withdraw->vendor_id)->decrement('pending_withdraw',
                $withdraw->amount);
            $withdraw->save();
    ...
    public function add_schedule(Request $request)
    {
        ...
        $temp = RestaurantSchedule::where('day', $request->day)->where('restaurant_id', $request->restaurant_id)
            ->where(function ($q) use ($request) {
                return $q->where(function ($query) use ($request) {
                    return $query->where('opening_time', '<=', $request->start_time)->where('closing_time', '>=',
                        $request->start_time);
                })->orWhere(function ($query) use ($request) {
                    return $query->where('opening_time', '<=', $request->end_time)->where('closing_time', '>=',
                        $request->end_time);
                });
            })
            ->first();
        ...
        $restaurant_schedule = RestaurantSchedule::insert([
        ...
        if ($request->query('key') != null) {
            ...
            $conversations = $conversations->where(function ($qu) use ($key) {
                ...
                $qu->whereHas('sender', function ($query) use ($key) {
                    foreach ($key as $value) {
                        $query->where('f_name', 'like', "%{$value}%")->orWhere('l_name', 'like',
                            "%{$value}%")->orWhere('phone', 'like', "%{$value}%");
                    }
                })
                    ->orWhereHas('receiver', function ($query1) use ($key) {
                        foreach ($key as $value) {
                            $query1->where('f_name', 'like', "%{$value}%")->orWhere('l_name', 'like',
                                "%{$value}%")->orWhere('phone', 'like', "%{$value}%");
                        }
                    });
            });
    ...
    public function conversation_view($conversation_id, $user_id)
    {
        ...
        return response()->json([
            'view' => view('admin-views.vendor.view.partials._conversations',
                compact('convs', 'user', 'receiver'))->render(),
        ]);
    }

    public function cash_transaction_export(Request $request)
    {
        $transaction = AccountTransaction::where('from_type', 'restaurant')->where('from_id',
            $request->restaurant)->get();
        if ($request->type == 'excel') {
